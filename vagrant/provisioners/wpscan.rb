...
`sudo bash --login -c "source /etc/profile.d/rvm.sh; rvm install ruby-#{wpscan_ruby_version}; rvm use ruby-#{wpscan_ruby_version}; rvm gemset create #{wpscan_gemset}; rvm use ruby-#{wpscan_ruby_version}@#{wpscan_gemset}; cd #{wpscan_root}; gem install bundler; bundle install --without test; rake install"`
puts 'complete.'
