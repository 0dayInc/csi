...
# puts `/bin/bash --login -c "source /etc/profile.d/rvm.sh; rvm use ruby-#{ruby_version}@#{rvm_gemset}; csi_jenkins_update_plugins -s '127.0.0.1' -d 8888 -U #{userland_user} -p #{userland_pass}"`
