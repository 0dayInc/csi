  ...
  module MSF
    ...
    module PostgresLogin
      ...
      def self.exploit(opts = {})
        ...
        if opts[:blank_passwords].nil?
          blank_passwords = false
        else
          blank_passwords = opts[:blank_passwords]
        end

        if opts[:bruteforce_speed].nil?
          bruteforce_speed = 5
        else
          bruteforce_speed = opts[:bruteforce_speed]
        end
        ...
        if opts[:db_all_creds].nil?
          db_all_creds = false
        else
          db_all_creds = opts[:db_all_creds]
        end

        if opts[:db_all_pass].nil?
          db_all_pass = false
        else
          db_all_pass = opts[:db_all_pass]
        end

        if opts[:db_all_users].nil?
          db_all_users = false
        else
          db_all_users = opts[:db_all_users]
        end
        ...
        if opts[:return_rowset].nil?
          return_rowset = true
        else
          return_rowset = opts[:return_rowset]
        end
        ...
        if opts[:stop_on_success].nil?
          stop_on_sucess = false
        else
          stop_on_sucess = opts[:stop_on_sucess]
        end
        ...
        if opts[:user_as_pass].nil?
          user_as_pass = false
        else
          user_as_pass = opts[:user_as_pass]
        end
