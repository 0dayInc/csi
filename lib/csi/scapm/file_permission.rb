...
module CSI
  module SCAPM
    # SCAPM Module used to identify the permissions
    ...
    module FilePermission
      ...
      def self.scan(opts = {})
        ...
        CSI::Plugins::FileFu.recurse_dir(dir_path: dir_path) do |entry|
          if ( File.file?(entry) && File.basename(entry) !~ /^csi.+(html|json|db)$/ && File.basename(entry) !~ /\.JS-BEAUTIFIED$/ ) && File.extname(entry) == ".pp"
            line_no_and_contents_arr = []
            filename_arr = []
            test_case_filter = %Q{ sed -e '/file {/,/}/!d;=' #{entry} }
            ...
            str = HTMLEntities.new.encode(numbered_str)
      ...
      def self.nist_800_53_requirements
        nist_800_53_requirements = {
          ...
          nist_800_53_uri: "https://web.nvd.nist.gov/view/800-53/Rev4/control?controlName=AC-6"
        }
