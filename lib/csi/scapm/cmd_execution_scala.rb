...
module CSI
  module SCAPM
    ...
    module CmdExecutionScala
      ...
      # CSI::SCAPM::CmdExecutionScala.scan(
      ...
      def self.scan(opts = {})
        ...
        CSI::Plugins::FileFu.recurse_dir(dir_path: dir_path) do |entry|
          if ( File.file?(entry) && File.basename(entry) !~ /^csi.+(html|json|db)$/ && File.basename(entry) !~ /\.JS-BEAUTIFIED$/ ) && File.extname(entry) == '.scala'
      ...
      def self.help
        puts "USAGE:
          cmd_execution_scala_arr = #{self}.scan(
            :dir_path => 'optional path to dir defaults to .',
            :git_repo_root_uri => 'optional http uri of git repo scanned'
          )
