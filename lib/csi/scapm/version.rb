...
module CSI
  module SCAPM
    # SCAPM Module used to detect version 
    ...
    module Version
      ...
      def self.scan(opts = {})
        ...
        CSI::Plugins::FileFu.recurse_dir(:dir_path => dir_path) do |entry|
          if ( File.file?(entry) && File.basename(entry) !~ /^csi.+(html|json|db)$/ && File.basename(entry) !~ /\.JS-BEAUTIFIED$/ )
            ...
            test_case_filter = %Q{grep -in "version\\s=\\s" #{entry} | grep -F '"'}
      ...
      def self.nist_800_53_requirements
        nist_800_53_requirements = {
          ...
          :nist_800_53_uri => "http://web.nvd.nist.gov/view/800-53/Rev4/control?controlName=RA-5"
        }
