...
module CSI
  module SCAPM
    # SCAPM Module used to identify command execution 
    ...
    module CmdExecutionPython
      ...
      # CSI::SCAPM::CmdExecutionPython.scan(
      ...
      def self.scan(opts = {})
        ...
        CSI::Plugins::FileFu.recurse_dir(dir_path: dir_path) do |entry|
          if ( File.file?(entry) && File.basename(entry) !~ /^csi.+(html|json|db)$/ && File.basename(entry) !~ /\.JS-BEAUTIFIED$/ ) && (File.extname(entry) == '.py' || File.extname(entry) == '.pyc' || File.extname(entry) == '.pyo' || File.extname(entry) == '.pyd')
      ...
      def self.help
        puts %Q{USAGE:
          cmd_execution_python_arr = #{self}.scan(
            :dir_path => 'optional path to dir defaults to .',
            :git_repo_root_uri => 'optional http uri of git repo scanned'
          )
