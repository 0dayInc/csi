...
if [[ -f "${csi_root}/Gemfile.lock" ]]; then
  rvmsudo rm $csi_root/Gemfile.lock
fi
...
if [[ $(uname -s) == "Darwin" ]]; then
  rvmsudo bundle config build.pg --with-pg-config=/opt/local/lib/postgresql96/bin/pg_config
  rvmsudo bundle config build.serialport --with-cflags=-Wno-implicit-function-declaration
fi
