    ...
    if File.exist?(config_path)
      ...
      config.vm.provider :aws do |aws, override|
        if csi_provider == 'aws'
          ...
          when 'us-east-1'
            aws.ami = 'ami-0f68aff3bc208fdc9'
          when 'us-east-2'
            aws.ami = 'ami-0c9f2fe6f77caf7fe'
          when 'us-west-1'
            aws.ami = 'ami-05a01b7c5c6ca9fb4'
          when 'us-west-2'
            aws.ami = 'ami-0bbe59d0a07734891'
          else
      ...
      config.vm.provision :shell, inline: "echo \"export CSI_ROOT='#{csi_root}'\" > /etc/profile.d/csi_envs.sh"
      config.vm.provision :shell, inline: "echo \"export CSI_PROVIDER='#{csi_provider}'\" >> /etc/profile.d/csi_envs.sh"
