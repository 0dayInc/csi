...
public class ApiServer {
    ...
    public ApiServer(String ip, int port, IBurpExtenderCallbacks callbacks) {
        ...
        post("/shutdown_session", (request, response) -> {
          try {
        ...
        post("/generate_scan_report", (request, response) -> {
          try {
            BScanReport report = gson.fromJson(request.body(), BScanReport.class);
            IScanIssue[] issues = callbacks.getScanIssues("");
            File output_path = new File(report.output_path);

            callbacks.generateScanReport(report.report_type, issues, output_path);
            response.status(201);
            return "";
          } catch (Exception e) {
            response.status(400);
